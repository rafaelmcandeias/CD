...
file_tag = 'NYC_collision'
filename = '../../data/firstDataset/NYC_collisions_features_selected.csv'
target = 'INJURED'

# TODO: SEPARAR TRAIN E TEST
...
savefig(f'../data/{file_tag}_gb_study_nyc.png')
show()
...
savefig(f'../data/{file_tag}_gb_best_nyc.png')
show()
...
savefig(f'../data/{file_tag}_gb_ranking_nyc.png')
...
savefig(f'../data/{file_tag}_gb_overfitting_study_nyc.png')
...
filename = '../../data/secondDataset/air_quality_features_selected.csv'
target = 'ALARM'
...
prd_tst = best_model.predict(tstX)
plot_evaluation_results(labels, trnY, prd_trn, tstY, prd_tst)
savefig(f'../data/{file_tag}_gb_best_aq.png')
show()
...
figure()
horizontal_bar_chart(elems, importances[indices], stdevs[indices], title='Gradient Boosting Features importance', xlabel='importance', ylabel='variables')
savefig(f'../data/{file_tag}_gb_ranking_aq.png')
...
for n in n_estimators:
    ...
    y_trn_values.append(eval_metric(trnY, prd_trn_Y))
plot_overfitting_study(n_estimators, y_trn_values, y_tst_values, name=f'GB_depth={max_depth}_lr={lr}', xlabel='nr_estimators', ylabel=str(eval_metric))
savefig(f'../data/{file_tag}_gb_overfitting_study_aq.png')
