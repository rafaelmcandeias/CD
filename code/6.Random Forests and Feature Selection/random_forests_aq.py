...
for scaling in ('minmax', 'zscore'):
    for balancing in ('undersampling', 'oversampling', 'SMOTEsampling'):
        ...
        train: DataFrame = read_csv(
            f'../../Lab4/data/balancing/{file_tag}_{scaling}_train_{balancing}.csv')
        ...
        test: DataFrame = read_csv(
            f'../../data/{dataset}/{file_tag}_{scaling}_test.csv')
        ...
        savefig(
            f'../data/images/{file_tag}_{scaling}_{balancing}_rf_study.png')
        ...
        savefig(
            f'../data/images/{file_tag}_{scaling}_{balancing}_rf_ranking.png')
        ...
        plot_overfitting_study(n_estimators, y_trn_values, y_tst_values,
                               name=f'RF_depth={max_depth}_vars={f}', xlabel='nr_estimators', ylabel=str(eval_metric))
        savefig(
            f'../data/images/{file_tag}_{scaling}_{balancing}_overfitting.png')
