...
for scaling in ('minmax', 'zscore'):
    for balancing in ('undersampling', 'oversampling', 'SMOTEsampling'):
        ...
        data = read_csv(
            f'../../Lab4/data/balancing/{dataset}/{filename}_{scaling}_train_{balancing}.csv')
        print(data.shape)
        ...
        savefig(
            f'../data/feature_selection/{filename}_fca_{THRESHOLD}_{scaling}_{balancing}.png')

        df = drop_redundant(data, drop)
        df.to_csv(
            f'../data/feature_selection/{filename}_dropped_{scaling}_{balancing}.csv')
