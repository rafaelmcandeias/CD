...
# FALTA SABER QUAL O CSV A USAR
file_tag = 'NYC_collisions'
filename = '../../data/firstDataset/NYC_collisions_fitted.csv'
target = 'INJURED'

# results achieved by a set of parameters combinations

# FALTA SEPARAR TRAIN E TEST
...
savefig(f'../data/{file_tag}_rf_study_nyc.png')
show()
...
savefig(f'../data/{file_tag}_rf_best_nyc.png')
show()
...
savefig(f'../data/{file_tag}_rf_ranking_nyc.png')
...
savefig(f'../data/{file_tag}_overfitting_study_nyc.png')
...
filename = '../../data/secondDataset/air_quality_fitted.csv'
target = 'ALARM'
...
for k in range(len(max_depths)):
    ...
    multiple_line_chart(n_estimators, values, ax=axs[0, k], title=f'Random Forests with max_depth={d}',
                           xlabel='nr estimators', ylabel='accuracy', percentage=True)
savefig(f'../data/{file_tag}_rf_study_aq.png')
show()
print('Best results with depth=%d, %1.2f features and %d estimators, with accuracy=%1.2f'%(best[0], best[1], best[2], last_best))
...
prd_tst = best_model.predict(tstX)
plot_evaluation_results(labels, trnY, prd_trn, tstY, prd_tst)
savefig(f'../data/{file_tag}_rf_best_aq.png')
show()
...
figure()
horizontal_bar_chart(elems, importances[indices], stdevs[indices], title='Random Forest Features importance', xlabel='importance', ylabel='variables')
savefig(f'../data/{file_tag}_rf_ranking_aq.png')
...
for n in n_estimators:
    ...
    y_trn_values.append(eval_metric(trnY, prd_trn_Y))
plot_overfitting_study(n_estimators, y_trn_values, y_tst_values, name=f'RF_depth={max_depth}_vars={f}', xlabel='nr_estimators', ylabel=str(eval_metric))
savefig(f'../data/{file_tag}_overfitting_study_aq.png')
