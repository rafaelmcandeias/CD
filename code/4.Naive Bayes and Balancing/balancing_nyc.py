...
for scalling in ("minmax", "zscore"):
    data = read_csv(f"../../Lab3/data/scalling/NYC_collisions_scaled_{scalling}.csv")
    X = data.drop(class_var, axis=1)
    X = X.drop(['CRASH_DATE', 'CRASH_TIME'], axis=1)
    y = data[class_var]
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
    data_train = concat([X_train, y_train], axis=1)
    data_train.to_csv(f'../../data/firstDataset/NYC_collisions_{scalling}_train.csv', index=False)
    data_test = concat([X_test, y_test], axis=1)
    data_test.to_csv(f'../../data/firstDataset/NYC_collisions_{scalling}_test.csv', index=False)

for scalling in ("minmax", "zscore"):
    original = read_csv(f'../../data/firstDataset/NYC_collisions_{scalling}_train.csv', sep=',', decimal='.')
    ...
    savefig(f'../data/balancing/images/NYC_collisions_{scalling}_train_balance.png')
    ...
    df_under.to_csv(f'../data/balancing/NYC_collisions_{scalling}_train_undersampling.csv', index=False)
    ...
    df_over.to_csv(f'../data/balancing/NYC_collisions_{scalling}_train_oversampling.csv', index=False)
    ...
    df_smote.to_csv(f'../data/balancing/NYC_collisions_{scalling}_train_SMOTEsampling.csv', index=False)
