...
def plot_line(xvalues: list, yvalues: list, ax: plt.Axes = None, title: str = '', xlabel: str = '', ylabel: str = '',
    ...
    ax = set_elements(ax=ax, title=title, xlabel=xlabel,
                      ylabel=ylabel, percentage=percentage)
...
def bar_chart(xvalues: list, yvalues: list, ax: plt.Axes = None, title: str = '', xlabel: str = '', ylabel: str = '',
    ...
    ax = set_elements(ax=ax, title=title, xlabel=xlabel,
                      ylabel=ylabel, percentage=percentage)
    set_locators(xvalues, ax=ax, rotation=rotation)
    ax.bar(xvalues, yvalues, edgecolor=cfg.LINE_COLOR,
           color=cfg.FILL_COLOR, tick_label=xvalues)
    for i in range(len(yvalues)):
        ax.text(i, yvalues[i] + TEXT_MARGIN,
                f'{yvalues[i]:.2f}', ha='center', fontproperties=FONT_TEXT)
...
def multiple_bar_chart(xvalues: list, yvalues: dict, ax: plt.Axes = None, title: str = '', xlabel: str = '', ylabel: str = '',
    ...
    for metric in yvalues:
        ax.bar(pos_group, yvalues[metric], width=width,
               edgecolor=cfg.LINE_COLOR, color=cfg.ACTIVE_COLORS[i])
        values = yvalues[metric]
        legend.append(metric)
        for k in range(len(values)):
            ax.text(pos_group[k], values[k] + TEXT_MARGIN,
                    f'{values[k]:.2f}', ha='center', fontproperties=FONT_TEXT)
...
def plot_confusion_matrix(cnf_matrix: ndarray, classes_names: ndarray, ax: plt.Axes = None, normalize: bool = False, title=''):
    ...
    set_elements(ax=ax, title=title, xlabel='Predicted label',
                 ylabel='True label', percentage=False)
...
def plot_roc_chart(models: dict, tstX: ndarray, tstY: ndarray, ax: plt.Axes = None, target: str = 'class'):
    ...
    ax.set_ylim(0.0, 1.0)
    set_elements(
        ax=ax, title=f'ROC chart for {target}', xlabel='FP rate', ylabel='TP rate')
...
def plot_tree(tree, variables: list, labels: list, out_filename: str):
    ...
    call(['dot', '-Tpng', out_filename+'.dot',
         '-o', out_filename+'.png', '-Gdpi=600'])
...
def plot_clusters(data, var1st, var2nd, clusters, centers: list, n_clusters: int, title: str,  ax: plt.Axes = None):
    if ax is None:
        ax = plt.gca()
    ax.scatter(data.iloc[:, var1st], data.iloc[:, var2nd],
               c=clusters, alpha=0.5, cmap=cfg.cmap_active)
...
def compute_centroids(data: DataFrame, labels: ndarray) -> list:
    ...
    for k in range(-1, n_clusters):
        if k != -1:
            ...
            centers[k] = [
                centers[k][j]/len(cluster) if len(cluster) > 0 else 0 for j in range(n_vars)]
        else:
            centers[k] = [0]*n_vars
