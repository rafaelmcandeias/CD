...
for balancing in ('undersampling', 'oversampling', 'SMOTEsampling'):
    data = read_csv(f'../../Lab4/data/air_quality_{balancing}.csv')
    X = data.drop(target, axis=1)
    y = data[target]
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
    data_train = concat([X_train, y_train], axis=1)
    data_train.to_csv(f'../data/knn/air_quality_{balancing}_train.csv')
    data_test = concat([X_test, y_test], axis=1)
    data_test.to_csv(f'../data/knn/air_quality_tabular_test.csv')

    train: DataFrame = read_csv(f'../data/knn/air_quality_{balancing}_train.csv')
    trnY: ndarray = train.pop(target).values
    trnX: ndarray = train.values
    labels = unique(trnY)
    labels.sort()

    test: DataFrame = read_csv(f'../data/knn/air_quality_tabular_test.csv')
    ...
    savefig(f'../data/knn/images/air_quality_{balancing}_study.png')
    ...
    savefig(f'../data/knn/images/air_quality_{balancing}_best.png')
